...
import logotipo from "@/assets/logotipo-creant.png";
...
const MARCAS = [
  { src: mGrannie, alt: "Grannie" },
  { src: mReyna, alt: "Los Budines de Reyna" },
  { src: mMalvada, alt: "Malvada Shoes" },
  { src: mNmo, alt: "NMO Perfumería" },
  { src: mResistencia, alt: "Resistencia" },
  { src: mAstegiano, alt: "Astegiano" },
  { src: mMagna, alt: "Magna Fitness" },
  { src: mCherry, alt: "Cherry Nails" },
];
...
/**
 * La obrera. Dibujada mirando a la derecha, que es hacia donde va el sendero.
 * Se reutiliza seis veces y cada una carga un servicio distinto.
 */
const Obrera = () => (
  <g id="obrera" fill="#2A1C13">
    <g transform="scale(-1,1) translate(-690,0)">
      ...
      <circle cx="282" cy="207" r="52" />
      <circle cx="358" cy="207" r="46" />
...
/** Cada obrera lleva un servicio al hombro. Las cargas van sin contorno:
 *  el color pleno se lee mejor que una silueta delineada. */
const CARGAS = [
  ...
  { escala: 0.045, retardo: "-10s", carga: (
      <g>
        <rect x="0" y="6" width="3.4" height="5" fill="#2A1C13" />
        <rect x="4.3" y="3" width="3.4" height="8" fill="#E8940C" />
        <rect x="8.6" y="0" width="3.4" height="11" fill="#C41230" />
      </g>
    ) },
  { escala: 0.039, retardo: "-15s", carga: (
      <g>
        <path d="M6 11.4 C-0.6 8.8, -0.6 2.4, 6 0.4 C12.6 2.4, 12.6 8.8, 6 11.4 Z" fill="#8C8F41" />
        <path d="M6 11.4 L6 1.2" stroke="#2A1C13" strokeWidth="1.4" opacity="0.6" />
      </g>
    ) },
  { escala: 0.043, retardo: "-20s", carga: <path d="M1.5 0.5 L11.5 5.5 L1.5 10.5 Z" fill="#C41230" /> },
  { escala: 0.036, retardo: "-25s", carga: (
      <g>
        <rect x="0" y="1" width="12" height="9" rx="1.2" fill="#E8940C" />
        <path d="M0.6 1.8 L6 6 L11.4 1.8" fill="none" stroke="#2A1C13" strokeWidth="2.2" strokeLinejoin="round" />
      </g>
    ) },
];
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* ================= SUPERFICIE ================= */}
      <section className="grano relative overflow-hidden pt-8">
        {/* Sendero de hormigas. Va en la franja libre de abajo: si cruzara el
            titular, caminarían tapadas por las letras. */}
        <svg
          className="pointer-events-none absolute bottom-9 left-0 right-0 z-[4] w-full"
          style={{ aspectRatio: "1440 / 130" }}
          viewBox="0 0 1440 130"
          aria-hidden="true"
        >
          <path
            d="M -40 96 C 250 62, 520 112, 800 74 S 1240 46, 1500 90"
            fill="none"
          ...
          {CARGAS.map((h, i) => (
            <g key={i} className="hormiga" style={{ animationDelay: h.retardo }}>
              <g transform={`translate(${(-340 * h.escala).toFixed(1)},${(-508 * h.escala).toFixed(1)}) scale(${h.escala})`}>
                <use href="#obrera" />
                <g transform="translate(130,-285) scale(40)">
                  <g className="carga">{h.carga}</g>
                </g>
              </g>
            </g>
          ))}
        ...
        <div className="container relative z-[3] mx-auto max-w-6xl px-6">
          <nav className="mb-16 flex items-center justify-between gap-4 md:mb-20">
            <img src={logotipo} alt="Creant" width="627" height="199" {...{ fetchpriority: "high" }} className="h-11 w-auto md:h-12" />
            <div className="hidden items-center gap-7 text-sm md:flex">
              <button onClick={irA("marcas")} className="border-b border-transparent pb-0.5 text-muted-foreground transition-colors hover:border-primary hover:text-foreground">Marcas</button>
              <button onClick={irA("servicios")} className="border-b border-transparent pb-0.5 text-muted-foreground transition-colors hover:border-primary hover:text-foreground">Servicios</button>
              <button onClick={irA("equipo")} className="border-b border-transparent pb-0.5 text-muted-foreground transition-colors hover:border-primary hover:text-foreground">Nosotros</button>
            </div>
            <Button onClick={irA("contacto")} className="rounded-none font-semibold">Hablemos</Button>
          </nav>
          ...
          <h1 className="mt-4 font-display text-[clamp(2.9rem,11vw,8.5rem)] font-extrabold uppercase leading-[0.86] tracking-[-0.045em]">
            ...
            <span className="block text-accent">
              ...
              <span className="relative inline-block">
                .
                {/* La hormiga colorada va al lado del punto, separada: comparten
                    el mismo rojo y superpuestas se leen como una sola mancha. */}
                <img
                  src={hormigaColorada}
                  alt=""
                  aria-hidden="true"
                  className="absolute"
                  style={{ left: "0.283em", bottom: "-0.166em", height: "0.30em", width: "0.219em", maxWidth: "none" }}
                />
              </span>
            </span>
          </h1>
          ...
          <motion.p
            variants={aparece} initial="hidden" animate="visible" custom={1}
          ...
          <motion.div variants={aparece} initial="hidden" animate="visible" custom={2} className="mb-16 mt-10 flex flex-wrap gap-3">
            ...
            <Button onClick={irA("marcas")} variant="outline" size="lg" className="h-14 rounded-none border-foreground px-8 text-base font-semibold">
              Ver las marcas
            </Button>
          </motion.div>
        </div>
        ...
        <svg className="relative z-[3] -mb-px block h-auto w-full" viewBox="0 0 1440 90" preserveAspectRatio="none" aria-hidden="true">
          <path
            d="M0,52 C90,30 150,74 246,58 C348,41 392,80 500,66 C600,53 660,86 762,70 C870,53 918,88 1030,72 C1130,58 1190,34 1290,52 C1352,63 1400,44 1440,50 L1440,90 L0,90 Z"
            fill="hsl(var(--tunel))"
          />
        </svg>
      </section>

      {/* ================= PRIMERA CÁMARA: LAS MARCAS ================= */}
      <section id="marcas" className="grano relative bg-tunel py-20 text-background">
        <div className="container mx-auto max-w-6xl px-6">
          <p className="font-mono text-[0.7rem] uppercase tracking-[0.22em] text-primary">Primera cámara</p>
          <h2 className="mt-3 font-display text-[clamp(1.9rem,5vw,3.4rem)] font-extrabold uppercase leading-[0.95] tracking-[-0.035em]">
            Salieron del hormiguero<br />listas para trabajar
          </h2>
          <p className="mb-10 mt-4 max-w-[32rem] text-background/70">
            Este no es un portfolio más. Son doce marcas que ya están afuera, funcionando.
          </p>
        </div>

        {/* Todas entran en la misma tinta: así no compiten entre ellas. */}
        <div className="overflow-hidden border-y border-background/15 py-8">
          <div className="cinta-pista flex w-max items-center gap-20" style={{ animation: "desfile 34s linear infinite" }}>
            {[...MARCAS, ...MARCAS].map((m, i) => (
              <img
                key={i}
                src={m.src}
                alt={i < MARCAS.length ? m.alt : ""}
      ...
      {/* ================= SERVICIOS ================= */}
      <section id="servicios" className="grano relative bg-secondary py-24">
        <div className="container mx-auto max-w-6xl px-6">
          <p className="font-mono text-[0.7rem] uppercase tracking-[0.22em] text-muted-foreground">Cómo trabajamos</p>
          <h2 className="mt-3 max-w-3xl font-display text-[clamp(1.9rem,5vw,3.4rem)] font-extrabold uppercase leading-[0.95] tracking-[-0.035em]">
            Estrategia, contenido y creatividad<br className="hidden md:block" /> trabajando juntos
          </h2>

          <div className="mt-14 grid gap-px bg-border md:grid-cols-3">
            {[
              { icono: PenTool, titulo: "Diseño estratégico", texto: "Identidad visual, branding y sistemas de marca pensados para que se apliquen bien en todos lados, no solo para que se vean lindos." },
              { icono: TrendingUp, titulo: "Marketing de crecimiento", texto: "Estrategia, gestión de redes y contenido con un plan detrás. Sabemos qué publicamos, para quién y por qué." },
              { icono: BarChart3, titulo: "Performance ads", texto: "Campañas en Meta y Google optimizadas con datos reales. Medimos cada consulta y sabemos de qué anuncio salió." },
            ].map((s, i) => (
              <motion.div
                key={s.titulo}
      ...
      <section id="equipo" className="grano relative py-24">
        <div className="container mx-auto max-w-6xl px-6">
          ...
          <p className="mt-5 max-w-3xl leading-relaxed text-muted-foreground">
            ...
            con la estrategia de pauta y conversión de Juan, y entre las dos cosas armamos marcas
            que se ven bien y además venden.
          </p>
            ...
            {[
              ...
              >
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  {p.tags.map((t) => (
                    <Badge key={t} variant="secondary" className="rounded-none text-xs font-normal">{t}</Badge>
                  ))}
                </div>
              </motion.div>
            ))}
      ...
      {/* ================= CONTACTO: EL FONDO DEL HORMIGUERO ================= */}
      <section id="contacto" className="camara grano relative bg-tunel py-24 text-background">
        <div className="container mx-auto max-w-4xl px-6 text-center">
          ...
          <div className="mt-12 flex flex-col items-center justify-center gap-5 text-sm text-background/60 sm:flex-row sm:gap-8">
            ...
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={trackWhatsapp("sección contacto")}
      ...
      <footer className="bg-tunel pb-10 text-center text-background/50">
        <div className="mb-4 flex items-center justify-center gap-6">
          ...
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={trackWhatsapp("pie")}
